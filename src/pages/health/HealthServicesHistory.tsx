...
import { ServiceType, RequestType } from '../../types';
...
export default function HealthServicesHistory() {
  ...
  const { patientVisits, dispensingHistory, referrals, followUps, appointments, healthRecords, schoolSettings, requests } = useData();
  ...
  const history: HistoryEntry[] = useMemo(() => {
    ...
    myRequests.forEach((r) => {
      const svcCfg = serviceConfig[r.type as ServiceType] ?? { label: r.type, icon: Stethoscope, color: 'text-teal-600', bg: 'bg-teal-50' };
      const details: string[] = [`Service: ${svcCfg.label}`];
      if (r.description) details.push(`Reason: ${r.description}`);
      if (r.reviewedBy) details.push(`Reviewed by: ${r.reviewedBy}`);
      if (r.reviewNotes) details.push(`Review Notes: ${r.reviewNotes}`);
      if (r.remarks) details.push(`Remarks: ${r.remarks}`);
      if (r.status === 'approved') details.push('Your request has been approved. Please proceed to the clinic.');
      if (r.status === 'rejected') details.push('Your request has been rejected. Please contact the clinic for more information.');
      if (r.status === 'forwarded' && r.forwardedTo) details.push(`Forwarded to: ${r.forwardedTo}`);

      const reqStatusMap: Record<string, string> = {
        pending: 'Pending',
        processing: 'Under Review',
        approved: 'Approved — Come to Clinic',
        rejected: 'Rejected',
        released: 'Released',
        forwarded: 'Forwarded',
      };
      ...
      entries.push({
        ...
        serviceLabel: 'Consultation Request',
        icon: svcCfg.icon,
        color: svcCfg.color,
        bg: svcCfg.bg,
        title: `${svcCfg.label} Request`,
        subtitle: `Submitted by ${r.userName}`,
        details,
        status: reqStatusMap[r.status] ?? r.status,
        statusVariant: r.status === 'approved' ? 'success' : r.status === 'rejected' ? 'danger' : r.status === 'pending' ? 'warning' : 'neutral',
      });
    });

    return entries.sort((a, b) => b.date.localeCompare(a.date));
  }, [myVisits, myDispensing, myReferrals, myFollowUps, myAppointments, myRequests]);

  const filtered = filterType === 'all' ? history : history.filter((e) => {
    if (filterType === 'consultation-request') return e.serviceLabel === 'Consultation Request';
    return e.type === filterType && e.serviceLabel !== 'Consultation Request';
  });
  ...
  const pendingRequests = myRequests.filter((r) => r.status === 'pending' || r.status === 'processing').length;
  ...
  const filterOptions = [
    ...
    { value: 'consultation-request', label: 'Consultation Requests' },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-sky-50 rounded-xl"><Stethoscope size={18} className="text-sky-500" /></div>
            <div><p className="text-2xl font-bold text-slate-800">{totalRequests}</p><p className="text-xs text-slate-500">Consultation Requests{pendingRequests > 0 ? ` (${pendingRequests} pending)` : ''}</p></div>
          </div>
        </div>
      </div>
