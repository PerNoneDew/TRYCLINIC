...
import { Search, Eye, Plus, Stethoscope, HeartPulse, Pill, ShieldPlus, Printer, Activity, Share2, Package } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { useData } from '../../context/DataContext';
import { PatientVisit, ServiceType, UserRole, User } from '../../types';
...
import { buildPhysicalExamPrintHtml } from '../../lib/physicalExamPrint';
...
export default function VisitListPage({ serviceType, title, description }: VisitListPageProps) {
  ...
  const { patientVisits, users, schoolSettings } = useData();
  ...
  return (
    <div className="space-y-5">
      ...
      <Modal isOpen={viewVisit !== null} onClose={() => setViewVisit(null)} title="Visit Details" size="lg" headerAction={viewVisit && serviceType === 'physical' && canRecord ? (
        <button onClick={() => {
          if (!viewVisit) return;
          const patient = users.find((u) => u.id === viewVisit.patientId);
          if (!patient) return;
          const formState: FormState = {
            patientId: viewVisit.patientId, patientName: viewVisit.patientName, patientRole: viewVisit.patientRole, department: viewVisit.department,
            chiefComplaint: viewVisit.chiefComplaint, symptoms: viewVisit.symptoms,
            temperature: viewVisit.temperature ?? '', bloodPressure: viewVisit.bloodPressure ?? '', heartRate: viewVisit.heartRate ?? '',
            respiratoryRate: viewVisit.respiratoryRate ?? '', weight: viewVisit.weight ?? '', height: viewVisit.height ?? '', oxygenSat: viewVisit.oxygenSat ?? '',
            assessment: viewVisit.assessment, diagnosis: viewVisit.diagnosis, treatmentProvided: viewVisit.treatmentProvided, firstAidTreatment: viewVisit.firstAidTreatment,
            medicineName: viewVisit.medicineName, medicineId: viewVisit.medicineId ?? '', dosage: viewVisit.dosage, quantity: viewVisit.quantity, unit: viewVisit.unit,
            instructions: viewVisit.instructions, remarks: viewVisit.remarks, clinical: viewVisit.clinicalData ?? {},
          };
          printHtml(buildPhysicalExamPrintHtml(formState, patient, schoolSettings?.schoolName ?? 'HEALTH SYS SFCG', schoolSettings?.schoolLogoPath ?? '/logo.png'));
        }} className="p-1.5 rounded-lg text-slate-400 hover:text-teal-600 hover:bg-teal-50 transition-colors" title="Print Examiner's Copy">
          <Printer size={18} />
        </button>
      ) : undefined}>
        {viewVisit && (
            ...
            {serviceType === 'first_aid' ? (
              <>
                ...
                <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                  <div className="flex items-center gap-2 mb-3"><ShieldPlus size={15} className="text-teal-500" /><h3 className="text-sm font-semibold text-slate-700">Record Incident</h3></div>
                  <div className="space-y-3">
                    <DetailItem label="Incident Description" value={viewVisit.chiefComplaint || '—'} />
                    <div className="grid grid-cols-2 gap-3">
                      <DetailItem label="Location" value={String(viewVisit.clinicalData?.incidentLocation ?? '—')} />
                      <DetailItem label="Time of Incident" value={String(viewVisit.clinicalData?.incidentTime ?? '—')} />
                    </div>
                    <DetailItem label="Injury Type" value={String(viewVisit.clinicalData?.injuryType ?? '—').replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())} />
                    <DetailItem label="Symptoms Observed" value={viewVisit.symptoms || '—'} />
                    {(viewVisit.temperature || viewVisit.bloodPressure || viewVisit.heartRate || viewVisit.oxygenSat) && (
                      <div>
                ...
                {/* First-Aid Treatment */}
                {viewVisit.firstAidTreatment && (
                  <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                    <div className="flex items-center gap-2 mb-3"><Stethoscope size={15} className="text-teal-500" /><h3 className="text-sm font-semibold text-slate-700">First-Aid Treatment</h3></div>
                    <DetailItem label="Treatment Given" value={viewVisit.firstAidTreatment} />
                  </div>
                )}
                ...
                {viewVisit.medicineName && (
                  <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                    <div className="flex items-center gap-2 mb-3"><Pill size={15} className="text-teal-500" /><h3 className="text-sm font-semibold text-slate-700">Medicine Given</h3></div>
                ...
                {/* Treatment Notes */}
                {viewVisit.treatmentProvided && (
                  <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                    <div className="flex items-center gap-2 mb-3"><Activity size={15} className="text-teal-500" /><h3 className="text-sm font-semibold text-slate-700">Treatment Notes</h3></div>
                    <DetailItem label="Notes" value={viewVisit.treatmentProvided} />
                  </div>
                )}

                {/* Referral */}
                {viewVisit.clinicalData?.referralRequired && viewVisit.clinicalData.referralRequired !== 'no' && (
                  <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                    <div className="flex items-center gap-2 mb-3"><Share2 size={15} className="text-teal-500" /><h3 className="text-sm font-semibold text-slate-700">Referral</h3></div>
                    <div className="space-y-3">
                      <DetailItem label="Referral Type" value={String(viewVisit.clinicalData.referralRequired).replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())} />
                      {viewVisit.clinicalData.referralDestination && <DetailItem label="Destination / Reason" value={String(viewVisit.clinicalData.referralDestination)} />}
                      {viewVisit.clinicalData.disposition && <DetailItem label="Disposition" value={String(viewVisit.clinicalData.disposition).replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())} />}
                    </div>
                  </div>
                )}
                ...
                {viewVisit.remarks && (
                  <div className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                    <div className="flex items-center gap-2 mb-3"><Package size={15} className="text-teal-500" /><h3 className="text-sm font-semibold text-slate-700">Remarks</h3></div>
                    <DetailItem label="Remarks" value={viewVisit.remarks} />
                  </div>
                )}
              </>
            ) : (
