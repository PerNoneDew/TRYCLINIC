...
export default function HealthForm() {
  ...
  const submitForm = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const request: HealthFormRequest = {
      ...
      formData: { ...form, fullName: form.fullName.trim() || currentUser.name }, status: 'pending', submittedAt: today, updatedAt: today,
    };
    await runWithFeedback(
      async () => { await persistHealthFormRequest(request); setForm(emptyForm); },
      { loadingTitle: 'Submitting...', successTitle: 'Health Form Submitted!', successMessage: 'The health officer and admin will review your submission.', autoCloseMs: 2500 },
    );
    setSubmitting(false);
  };
  ...
  return (
    <div className="space-y-5">
      ...
      {/* My submissions list with print for approved */}
      {visibleRequests.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100">
            <h3 className="font-semibold text-slate-800">My Health Form Submissions</h3>
            <p className="text-xs text-slate-400 mt-0.5">Track the status of your submissions. Approved forms can be printed for your records.</p>
          </div>
          <div className="divide-y divide-slate-50">
            {visibleRequests.map((req) => (
              <div key={req.id} className="flex items-center justify-between px-5 py-4">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`p-2 rounded-xl shrink-0 ${req.status === 'approved' ? 'bg-emerald-50' : req.status === 'pending' ? 'bg-amber-50' : 'bg-rose-50'}`}>
                    {req.status === 'approved' ? <CheckCircle size={16} className="text-emerald-500" /> : req.status === 'pending' ? <Clock size={16} className="text-amber-500" /> : <XCircle size={16} className="text-rose-500" />}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-slate-700 truncate">{req.formData.fullName || req.userName}</p>
                    <p className="text-xs text-slate-400">Submitted {req.submittedAt}{req.reviewedBy && ` · Reviewed by ${req.reviewedBy}`}</p>
                    {req.reviewNotes && <p className="text-xs text-slate-500 mt-1 italic">"{req.reviewNotes}"</p>}
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <Badge label={req.status.charAt(0).toUpperCase() + req.status.slice(1)} variant={statusVariant(req.status)} />
                  {req.status === 'approved' && (
                    <button onClick={() => printHtml(buildFormHtml(req.formData))} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium bg-teal-500 hover:bg-teal-600 text-white transition-colors">
