...
export default function RequestManagement() {
  ...
  const updateStatus = (id: string, status: RequestStatus, notes?: string, rmks?: string, ft?: string, fr?: string) => {
    ...
    if (req) {
      ...
      if (status === 'approved') {
        createNotification({
          title: 'Request Approved — Come to the Clinic',
          message: `Your ${svcLabel} request has been APPROVED. Please come to the clinic. ${rmks ? rmks : ''}`,
        ...
        createNotification({
          title: 'Request Rejected — No Need to Visit',
          message: `Your ${svcLabel} request has been REJECTED. There is no need to visit the clinic for this request. ${notes ? `Reason: ${notes}` : ''}`,
          recipientRoles: (req.userRole ? [req.userRole] : []) as Notification['recipientRoles'],
          recipientIds: [req.userId],
          sentBy: currentUser.name,
          type: 'rejection',
