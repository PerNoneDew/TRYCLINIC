...
type RolePermissionRow = {
  id: string; role: string; permissions: string[]; updated_at: string; updated_by: string | null;
};
...
export async function upsertRolePermissions(role: string, permissions: string[], updatedBy?: string): Promise<void> {
  const { error } = await supabase.from('role_permissions').upsert({
    role, permissions, updated_by: updatedBy ?? null, updated_at: new Date().toISOString(),
  });
  if (error) throw error;
}
