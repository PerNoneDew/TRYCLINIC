...
import {
  ...
  ShoppingCart, Truck, ArrowLeftRight, Upload, UserCog,
  Lock, GraduationCap, Briefcase, HeartPulse, ShieldPlus, Activity,
} from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { useData } from '../../context/DataContext';
import { Page } from '../../types';
import { canAccess, PermissionKey, pagePermissions } from '../../lib/permissions';
...
const adminNavGroups: NavGroup[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { id: 'request-management', label: 'Consultation Requests', icon: Stethoscope },
    ],
  },
  {
...
const regularNavGroups: NavGroup[] = [
  {
    id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard,
  },
  {
    ...
    children: [
      ...
      { id: 'request-consultation', label: 'Request Consultation', icon: Stethoscope },
    ],
  },
  {
...
export default function Sidebar({ currentPage, onNavigate }: SidebarProps) {
  ...
  const useAdminNav = currentUser.role === 'admin' || currentUser.role === 'staff';
  const allGroups = useAdminNav ? adminNavGroups : regularNavGroups;

  const pageAllowed = (page: Page): boolean => canAccess(page, currentUser.role, rolePermissions);

  const visibleGroups = allGroups.filter((g) => {
    if (g.children) {
      return g.children.some((c) => pageAllowed(c.id)) || pageAllowed(g.id);
    }
    return pageAllowed(g.id);
  });
            ...
            <div key={group.id}>
              ...
              {hasChildren && isExpanded && (
                <div className="mt-0.5 ml-3 pl-3 border-l border-slate-700/50 space-y-0.5">
                  {group.children!.filter((child) => pageAllowed(child.id)).map((child) => {
