...
export default function PatientVisitForm({ serviceType, isOpen, onClose, editVisit, presetPatient }: PatientVisitFormProps) {
  ...
  const { users, inventory, persistPatientVisit, persistMedicine, persistDispensing, persistStockTransaction, schoolSettings } = useData();
  ...
  const handleSave = async () => {
    ...
    if (serviceType !== 'physical' && !form.chiefComplaint.trim()) return;
    ...
    if (form.medicineId && form.quantity > 0) {
      ...
      if (med) {
        const previousQty = editVisit?.medicineId === form.medicineId ? (editVisit.quantity ?? 0) : 0;
        const availableForDeduct = med.quantity + previousQty;
        if (form.quantity > availableForDeduct) {
          alert(`Insufficient stock for ${med.name}. Available: ${availableForDeduct} ${med.unit}, requested: ${form.quantity} ${form.unit}.`);
          return;
        }
      }
    }
    ...
    const success = await runWithFeedback(
      async () => {
        ...
        if (form.medicineId && form.quantity > 0) {
          ...
          // In edit mode, restore the previously deducted quantity first
          const previousQty = (editVisit?.medicineId === form.medicineId) ? (editVisit.quantity ?? 0) : 0;
          const netDeduct = form.quantity - previousQty;

          if (netDeduct !== 0) {
            const newQty = med.quantity - netDeduct;
            await persistMedicine({ ...med, quantity: newQty, lastUpdated: now });

            // Record stock transaction for audit trail
            await persistStockTransaction({
              id: `stx${Date.now()}`,
              itemId: med.id,
              itemName: med.name,
              itemType: 'medicine',
              transactionType: 'issuance',
              quantity: Math.abs(netDeduct),
              unit: med.unit,
              reason: `${serviceLabel[serviceType]} — ${form.patientName} (${form.chiefComplaint || 'no complaint noted'})`,
              recordedBy: currentUser?.name ?? '',
              recordedAt: now,
              notes: netDeduct < 0 ? 'Stock restored from edited visit' : undefined,
            });
          }

          // Record dispensing history (only for new visits, not edits)
          if (!editVisit) {
      ...
      },
      {
        ...
        errorTitle: 'Failed to save visit',
        autoCloseMs: 0,
      },
    );
    if (success) {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`${editVisit ? 'Edit' : 'Record'} ${serviceLabel[serviceType]}`} size="xl">
      <div className="space-y-5">
        ...
        {selectedPatient && (
          <>
            ...
            {serviceType === 'first_aid' && (
              <>
                <Section title="Record Incident" icon={ShieldPlus}>
                  <div className="space-y-3">
                    ...
                    <div>
                      <p className="text-xs font-medium text-slate-500 mb-2">Vital Signs</p>
                ...
                <Section title="First-Aid Treatment" icon={Stethoscope}>
                ...
                <Section title="Treatment Notes" icon={Activity}>
                  <Field label="Treatment Notes">
                    <textarea value={form.treatmentProvided} onChange={(e) => setForm({ ...form, treatmentProvided: e.target.value })} rows={3} className={inputCls} placeholder="Additional treatment notes, observations, or follow-up instructions..." />
                  </Field>
                </Section>

                <Section title="Referral" icon={Share2}>
                  <div className="space-y-3">
        ...
        <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
          ...
          <button onClick={handleSave} disabled={!form.patientId || (serviceType !== 'physical' && !form.chiefComplaint.trim())} className="px-4 py-2 text-sm font-medium bg-teal-500 hover:bg-teal-600 disabled:bg-teal-300 text-white rounded-xl transition-colors">{editVisit ? 'Save Changes' : 'Submit'}</button>
        </div>
      </div>
    </Modal>
  );
