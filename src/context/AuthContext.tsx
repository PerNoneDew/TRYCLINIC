...
function makeAuditLog(partial: Omit<AuditLog, 'id' | 'timestamp' | 'ipAddress'>): AuditLog {
  return {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (elapsed >= SESSION_DURATION) {
        if (currentUser) {
          const timeoutLog = makeAuditLog({
            userId: currentUser.id, userName: currentUser.name, action: 'Session expired',
            module: 'Authentication', details: `Auto-logout due to inactivity for ${currentUser.email}`, type: 'logout',
          });
          db.upsertAuditLogsAll([timeoutLog]).catch((e) => console.error('persist timeout audit:', e));
        }
        setCurrentUser(null);
  ...
  const login = useCallback((email: string, password: string): boolean => {
    ...
    if (!expectedPassword || expectedPassword !== password) {
      const failedLog = makeAuditLog({
        userId: 'unknown', userName: email, action: 'Failed login attempt',
        module: 'Authentication', details: `Failed login attempt for ${normalizedEmail}`, type: 'login',
      });
      db.upsertAuditLogsAll([failedLog]).catch((e) => console.error('persist failed login audit:', e));
      return false;
    }
    const user = users.find((u) => normalizeEmail(u.email) === normalizedEmail);
    if (!user || user.status === 'inactive') {
      const failedLog = makeAuditLog({
        userId: user?.id ?? 'unknown', userName: user?.name ?? email, action: 'Failed login attempt',
        module: 'Authentication', details: `Login denied — account ${normalizedEmail} is ${user ? 'inactive' : 'not found'}`, type: 'login',
      });
      db.upsertAuditLogsAll([failedLog]).catch((e) => console.error('persist failed login audit:', e));
      return false;
    }
    lastActivity.current = Date.now();
    setCurrentUser(user);
    const successLog = makeAuditLog({
      userId: user.id, userName: user.name, action: 'User logged in',
      module: 'Authentication', details: `Successful login for ${normalizedEmail} (${user.role})`, type: 'login',
    });
    db.upsertAuditLogsAll([successLog]).catch((e) => console.error('persist login audit:', e));
    return true;
  }, [credentials, users]);

  const logout = useCallback(() => {
    if (currentUser) {
      const logoutLog = makeAuditLog({
        userId: currentUser.id, userName: currentUser.name, action: 'User logged out',
        module: 'Authentication', details: `Logout for ${currentUser.email}`, type: 'logout',
      });
      db.upsertAuditLogsAll([logoutLog]).catch((e) => console.error('persist logout audit:', e));
    }
    setCurrentUser(null);
    setSessionTimeout(null);
    sessionStorage.removeItem(SESSION_KEY);
