...
import { HealthRecord, Request, Medicine, Expense, Notification, AuditLog, BackupRecord, MedicineDispensing, User, DataSnapshot, HealthFormRequest, PatientVisit, Referral, FollowUp, Appointment, MedicalSupply, StockTransaction, Supplier, Purchase, SchoolSettings, UserRole } from '../types';
import * as db from '../lib/db';
import { RolePermissionMap, PermissionKey, defaultPermissionMap, defaultRolePermissions } from '../lib/permissions';
...
interface DataContextType {
  ...
  persistRolePermissions: (role: UserRole, permissions: PermissionKey[], updatedBy?: string) => Promise<void>;
}
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const [rolePermissions, setRolePermissions] = useState<RolePermissionMap>(defaultPermissionMap);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [hr, req, med, exp, notif, audit, backup, disp, snap, hfr, pv, ref, fu, appt, ms, st, sup, pur, ss, rp] = await Promise.all([
          ...
          db.fetchRolePermissions(),
        ]);
        setSchoolSettings(ss);
        // Merge DB permissions over defaults so any role not yet in DB keeps its defaults
        const merged = defaultPermissionMap();
        (Object.keys(merged) as UserRole[]).forEach((role) => {
          if (rp[role] && rp[role].length > 0) {
            merged[role] = new Set(rp[role] as PermissionKey[]);
          }
        });
  ...
  const persistRolePermissions = useCallback(async (role: UserRole, permissions: PermissionKey[], updatedBy?: string): Promise<void> => {
    setRolePermissions((prev) => ({ ...prev, [role]: new Set(permissions) }));
    await db.upsertRolePermissions(role, permissions, updatedBy);
  }, []);
